 ...
 * @delegate ElementRefresh.refreshElements
 */
export async function refreshElements(wizard) {
    return ElementRefresh.refreshElements(wizard, {
        updateNavigationContext: updateNavigationContext,
        updateScreenshotDisplay: updateScreenshotDisplay
    });
}
 ...
 * @delegate ElementRefresh.refreshAfterAction
 */
export async function refreshAfterAction(wizard, delayMs = 500) {
    return ElementRefresh.refreshAfterAction(wizard, delayMs, {
        updateNavigationContext: updateNavigationContext,
        updateScreenshotDisplay: updateScreenshotDisplay
    });
}
...
export async function handleTreeSensor(wizard, element) {
    ...
    const Dialogs = await import('./flow-wizard-dialogs.js?v=0.4.0-beta.4');
...
export function renderFilteredElements(wizard) {
    ...
    panel.querySelectorAll('.btn-tap').forEach(btn => {
        btn.addEventListener('click', async () => {
            const index = parseInt(btn.dataset.index);
            const ElementActions = await import('./flow-wizard-element-actions.js?v=0.4.0-beta.4');
