...
export async function handleTreeSensor(wizard, element) {
    ...
    const Dialogs = await import('./flow-wizard-dialogs.js?v=0.4.0-beta.2.7');
...
export function renderFilteredElements(wizard) {
    ...
    panel.querySelectorAll('.btn-tap').forEach(btn => {
        btn.addEventListener('click', async () => {
            const index = parseInt(btn.dataset.index);
            const ElementActions = await import('./flow-wizard-element-actions.js?v=0.4.0-beta.2.7');
